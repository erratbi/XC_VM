...
$xtreampiPageScripts = ['assets/xtreampi/mag.js'];
$xtreampiMag = is_array($rDevice ?? null) ? $rDevice : [];
$xtreampiLine = is_array($xtreampiMag['user'] ?? null) ? $xtreampiMag['user'] : [];
$xtreampiEditing = intval($xtreampiMag['mag_id'] ?? 0) > 0;
...
$xtreampiValue = static fn(array $source, string $key, string $default = ''): string => htmlspecialchars((string) ($source[$key] ?? $default), ENT_QUOTES, 'UTF-8');
?>
<section class="sc-line-editor" data-sc-mag-editor>
 <div class="sc-page-heading"><div><p class="sc-eyebrow">Device Management</p><h1><?php echo $xtreampiEditing ? 'Edit MAG Device' : 'Add MAG Device'; ?></h1></div><div class="sc-page-actions"><a class="sc-button sc-button-secondary" href="mags"><i class="fe-chevron-left"></i> Back to MAG devices</a></div></div>
 <form class="sc-form sc-line-form" action="post.php?action=mag&amp;referer=mags" method="post" enctype="multipart/form-data" data-status-invalid-mac="<?php echo intval(STATUS_INVALID_MAC); ?>" data-status-existing-mac="<?php echo intval(STATUS_EXISTS_MAC); ?>" data-status-invalid-date="<?php echo intval(STATUS_INVALID_DATE); ?>">
  <?php if ($xtreampiEditing): ?><input type="hidden" name="edit" value="<?php echo intval($xtreampiMag['mag_id']); ?>"><?php endif; ?><input type="hidden" name="bouquets_selected" value="[]">
  <section class="sc-form-section"><h2>Device &amp; Account</h2><div class="sc-form-grid">
   <label>MAC Address<input type="text" name="mac" required autocomplete="off" inputmode="text" maxlength="17" pattern="[0-9A-Fa-f]{2}(?::[0-9A-Fa-f]{2}){5}" placeholder="00:1A:79:00:00:00" value="<?php echo $xtreampiValue($xtreampiMag, 'mac', '00:1A:79:'); ?>" data-sc-mac-input></label>
   <label>Adult PIN<input type="text" name="parent_password" inputmode="numeric" maxlength="4" value="<?php echo $xtreampiValue($xtreampiMag, 'parent_password', '0000'); ?>"></label>
   ...
   <label>Admin Notes<textarea name="admin_notes" rows="3"><?php echo $xtreampiValue($xtreampiLine, 'admin_notes'); ?></textarea></label>
   <label>Reseller Notes<textarea name="reseller_notes" rows="3"><?php echo $xtreampiValue($xtreampiLine, 'reseller_notes'); ?></textarea></label>
  </div><div class="sc-mag-policies"><h3>Device Policies</h3><div class="sc-package-options sc-line-switches">
   <label class="sc-selection-card"><input type="checkbox" name="is_trial"<?php echo !empty($xtreampiLine['is_trial']) ? ' checked' : ''; ?>><span><strong>Trial device</strong><small>Mark this device as a trial account.</small></span></label>
   <label class="sc-selection-card"><input type="checkbox" name="is_isplock"<?php echo !empty($xtreampiLine['is_isplock']) ? ' checked' : ''; ?>><span><strong>Lock to ISP</strong><small>Restrict access to the detected provider.</small></span></label>
   <label class="sc-selection-card"><input type="checkbox" name="lock_device"<?php echo !$xtreampiEditing || !empty($xtreampiMag['lock_device']) ? ' checked' : ''; ?>><span><strong>Lock device identity</strong><small>Prevent the device identifiers from changing.</small></span></label>
  </div></div></section>
  <?php if ($xtreampiEditing): ?><section class="sc-form-section"><h2>Reported Device Information</h2><div class="sc-form-grid">
   <label>Line Username<input type="text" name="username" value="<?php echo $xtreampiValue($xtreampiLine, 'username'); ?>"></label><label>Line Password<input type="text" name="password" value="<?php echo $xtreampiValue($xtreampiLine, 'password'); ?>"></label>
   <?php foreach ([['sn','Serial Number'],['stb_type','STB Type'],['image_version','Image Version'],['hw_version','Hardware Version'],['device_id','Primary Device ID'],['device_id2','Secondary Device ID'],['ver','Firmware Version']] as [$key,$label]): ?><label><?php echo $label; ?><input type="text" name="<?php echo $key; ?>" value="<?php echo $xtreampiValue($xtreampiMag, $key); ?>"></label><?php endforeach; ?>
  </div></section><?php endif; ?>
  <section class="sc-form-section"><h2>Connection Options</h2><div class="sc-form-grid">
   <label>Forced Server<select name="force_server_id"><option value="0">Disabled</option><?php foreach ($xtreampiServers as $server): ?><option value="<?php echo intval($server['id']); ?>"<?php echo intval($xtreampiLine['force_server_id'] ?? 0) === intval($server['id']) ? ' selected' : ''; ?>><?php echo htmlspecialchars((string) $server['server_name'], ENT_QUOTES, 'UTF-8'); ?></option><?php endforeach; ?></select></label>
