		...
		if ($_PAGE == 'movie' || $_PAGE == 'episodes') {
			...
			if ($_PAGE != 'episodes') {
				...
				if ($rLegacy) {
					...
					echo '    function openPlayer(rID = 0) {' . "\r\n" . '        window.currentID = rID;' . "\r\n" . '        if (!window.vjsPlayer) { window.vjsPlayer = videojs("now__playing__player", { fluid: true, autoplay: true }); window.vjsPlayer.on("error", showError); }' . "\r\n" . '        window.vjsPlayer.src({ src: window.rURLs[rID], type: (window.rURLs[rID].indexOf(".m3u8") !== -1 ? "application/x-mpegURL" : "video/mp4") });' . "\r\n" . '        var rTracks = window.vjsPlayer.remoteTextTracks(); for (var i = rTracks.length - 1; i >= 0; i--) { window.vjsPlayer.removeRemoteTextTrack(rTracks[i]); }' . "\r\n" . '        if (window.rSubtitles[rID]) { for (var i = 0; i < window.rSubtitles[rID].length; i++) { window.vjsPlayer.addRemoteTextTrack({ src: window.rSubtitles[rID][i].file, label: window.rSubtitles[rID][i].label, kind: "subtitles" }, false); } }' . "\r\n" . '        $("#player_row").slideDown(250, function() {' . "\r\n" . "            \$('html,body').animate({" . "\r\n" . '                scrollTop: $("#player_row").offset().top - 100' . "\r\n" . '            });' . "\r\n" . '        });' . "\r\n" . '    }' . "\r\n" . '    ';
				}
			}
			...
			if ($rLegacy) {
				...
				echo '    function showError() {' . "\r\n" . '        var rError = window.vjsPlayer ? window.vjsPlayer.error() : null;' . "\r\n" . '        ';

				if ($_PAGE == 'movie') {
					echo '        $("#player_row").slideUp(250);' . "\r\n" . "        \$('html,body').animate({" . "\r\n" . '            scrollTop: 0' . "\r\n" . '        });' . "\r\n" . '        $("#player__error").html("Error " + (rError ? rError.code : "") + " - " + (rError ? rError.message : "Playback failed"));' . "\r\n" . '        $("#player__error").slideDown(250);' . "\r\n" . '        ';
				} else {
			...
			if ($rLegacy) {
				...
				if ($_PAGE != 'movie') {
					...
					echo '        var rPlayer = videojs("now__playing__player", { fluid: true, autoplay: true });' . "\r\n" . '        rPlayer.src({ src: window.rURLs[0], type: (window.rURLs[0].indexOf(".m3u8") !== -1 ? "application/x-mpegURL" : "video/mp4") });' . "\r\n" . '        var rSubs = window.rSubtitles[0]; if (rSubs) { for (var i = 0; i < rSubs.length; i++) { rPlayer.addRemoteTextTrack({ src: rSubs[i].file, label: rSubs[i].label, kind: "subtitles" }, false); } }' . "\r\n" . "        rPlayer.on('error', showError);" . "\r\n" . '        window.vjsPlayer = rPlayer;' . "\r\n" . '        ';
				}
			}
