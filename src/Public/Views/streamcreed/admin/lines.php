...
$streamcreedServers = ServerRepository::getAll();
$streamcreedPlaylistBase = rtrim((string) ($streamcreedServers[SERVER_ID]['site_url'] ?? ''), '/');
$streamcreedOutputDevices = LineRepository::getOutputDevices();
?>
<section class="sc-subscriptions" data-sc-subscriptions data-endpoint="table" data-default-entries="<?php echo $streamcreedDefaultEntries; ?>" data-can-edit="<?php echo $streamcreedCanEditLines ? '1' : '0'; ?>" data-can-view-connections="<?php echo $streamcreedCanViewConnections ? '1' : '0'; ?>" data-can-fingerprint="<?php echo $streamcreedCanFingerprint ? '1' : '0'; ?>">
...
<dialog class="sc-playlist-dialog" data-sc-playlist-dialog data-playlist-base="<?php echo htmlspecialchars($streamcreedPlaylistBase, ENT_QUOTES, 'UTF-8'); ?>">
	<form method="dialog">
		...
		<div class="sc-dialog-body">
			<label>Format<select data-sc-playlist-format><option value="">Choose a format</option><?php foreach ($streamcreedOutputDevices as $streamcreedDevice): ?><optgroup label="<?php echo htmlspecialchars((string) $streamcreedDevice['device_name'], ENT_QUOTES, 'UTF-8'); ?>"><?php foreach ([['?output=hls', 'HLS'], ['', 'MPEGTS'], ['?output=rtmp', 'RTMP']] as [$streamcreedSuffix, $streamcreedLabel]): ?><option value="<?php echo htmlspecialchars((string) $streamcreedDevice['device_key'] . $streamcreedSuffix, ENT_QUOTES, 'UTF-8'); ?>" data-copy-text="<?php echo htmlspecialchars((string) ($streamcreedDevice['copy_text'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>"><?php echo htmlspecialchars((string) $streamcreedDevice['device_name'] . ' - ' . $streamcreedLabel, ENT_QUOTES, 'UTF-8'); ?></option><?php endforeach; ?></optgroup><?php endforeach; ?></select></label>
			<fieldset><legend>Limit output</legend><div class="sc-playlist-types"><?php foreach ([['live','Live streams'],['movie','Movies'],['created_live','Created channels'],['radio_streams','Radio stations'],['series','TV series']] as [$streamcreedKey,$streamcreedLabel]): ?><label class="sc-check"><input type="checkbox" value="<?php echo $streamcreedKey; ?>" data-sc-playlist-output><span><?php echo $streamcreedLabel; ?></span></label><?php endforeach; ?></div></fieldset>
