...
$streamcreedSelectedServer = isset($_GET['server_id']) ? intval($_GET['server_id']) : null;
$streamcreedVisibleServers = array_values(array_filter($rOrderedServers, static function (array $server) use ($streamcreedSelectedServer): bool {
	if (empty($server['enabled'])) {
		return false;
	}
	return $streamcreedSelectedServer === null || intval($server['id']) === $streamcreedSelectedServer;
}));
$streamcreedStatsUrl = './api?action=stats';
if ($streamcreedSelectedServer !== null) {
	$streamcreedStatsUrl .= '&server_id=' . $streamcreedSelectedServer;
}
$streamcreedServerType = static function (array $server): string {
...
<?php else: ?>
	<section class="sc-dashboard" data-sc-dashboard data-stats-url="<?php echo htmlspecialchars($streamcreedStatsUrl, ENT_QUOTES, 'UTF-8'); ?>" data-selected-server="<?php echo $streamcreedSelectedServer ?? ''; ?>">
			...
			<label class="sc-server-filter">
				...
				<select data-sc-server-filter>
					...
					<?php foreach ($rOrderedServers as $server): ?>
						...
						<option value="<?php echo intval($server['id']); ?>"<?php echo $streamcreedSelectedServer === intval($server['id']) ? ' selected' : ''; ?>><?php echo htmlspecialchars((string) $server['server_name'], ENT_QUOTES, 'UTF-8'); ?></option>
					<?php endforeach; ?>
				</select>
			</label>
		</div>
		...
		<div class="sc-server-grid">
			<?php foreach ($streamcreedVisibleServers as $server): ?>
				...
				<article class="sc-server-card<?php echo empty($server['server_online']) ? ' is-offline' : ''; ?>" data-sc-server="<?php echo $serverID; ?>">
					<header class="sc-server-card-header">
						<div>
							...
							<span><?php echo htmlspecialchars($streamcreedServerType($server), ENT_QUOTES, 'UTF-8'); ?></span>
						</div>
			...
			<?php endforeach; ?>
			<?php if ($streamcreedVisibleServers === []): ?>
