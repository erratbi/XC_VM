...
$streamcreedPageScripts = ['assets/streamcreed/servers.js'];
$streamcreedServers = array_values(array_filter($rServers, static fn(array $server): bool => intval($server['server_type'] ?? 0) === 0));
usort($streamcreedServers, static fn(array $left, array $right): int => intval($left['order'] ?? $left['id']) <=> intval($right['order'] ?? $right['id']));
$streamcreedCanEditServers = Authorization::check('adv', 'edit_server');
$streamcreedCanOrderServers = Authorization::check('adv', 'server_order');
$streamcreedCanMonitorProcesses = Authorization::check('adv', 'process_monitor');
...
$streamcreedMainVersion = (string) ($rServers[SERVER_ID]['xc_vm_version'] ?? XC_VM_VERSION);

$streamcreedStatus = static function (array $server): array {
...
<section class="sc-inventory" data-sc-server-inventory>
	...
	<div class="sc-inventory-grid">
		<?php foreach ($streamcreedServers as $server): ?>
			...
			[$statusKey, $statusLabel] = $streamcreedStatus($server);
			...
			$connections = $streamcreedRedisHandler ? intval($server['connections'] ?? 0) : intval(ConnectionTracker::getLiveConnections($serverID));
			...
			$versionMismatch = $version !== '' && $streamcreedMainVersion !== '' && $version !== $streamcreedMainVersion;
			...
			<article class="sc-inventory-card" data-sc-inventory-card data-status="<?php echo htmlspecialchars($statusKey, ENT_QUOTES, 'UTF-8'); ?>" data-search="<?php echo htmlspecialchars($searchValue, ENT_QUOTES, 'UTF-8'); ?>">
				...
				<footer class="sc-inventory-card-footer">
					...
					<div>
						<?php if ($streamcreedCanViewConnections): ?><a href="live_connections?server=<?php echo $serverID; ?>">Connections</a><?php endif; ?>
						<?php if ($streamcreedCanMonitorProcesses): ?><a href="process_monitor?server=<?php echo $serverID; ?>">Processes</a><?php endif; ?>
						<a href="server_view?id=<?php echo $serverID; ?>">Monitor</a>
						<?php if ($streamcreedCanEditServers): ?><a class="sc-card-primary-action" href="server?id=<?php echo $serverID; ?>">Edit</a><?php endif; ?>
					</div>
				</footer>
			</article>
		<?php endforeach; ?>
