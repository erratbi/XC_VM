				...
				<tbody data-sc-bouquet-rows>
					<?php foreach ($streamcreedBouquets as $streamcreedBouquet): ?>
						...
						<tr data-sc-bouquet-row data-bouquet-id="<?php echo $streamcreedID; ?>" data-search="<?php echo htmlspecialchars(strtolower($streamcreedID . ' ' . $streamcreedName), ENT_QUOTES, 'UTF-8'); ?>">
							...
							<td class="sc-table-actions"><?php if ($streamcreedCanEditBouquet): ?><a class="sc-row-action" href="bouquet?id=<?php echo $streamcreedID; ?>" title="Edit bouquet"><i class="fe-edit-2" aria-hidden="true"></i></a><a class="sc-row-action" href="bouquet_sort?id=<?php echo $streamcreedID; ?>" title="Reorder bouquet content"><i class="fe-list" aria-hidden="true"></i></a><a class="sc-row-action" href="bouquet?duplicate=<?php echo $streamcreedID; ?>" title="Duplicate bouquet"><i class="fe-copy" aria-hidden="true"></i></a><button class="sc-row-action" type="button" title="Delete bouquet" aria-label="Delete bouquet" data-sc-bouquet-delete="<?php echo $streamcreedID; ?>"><i class="fe-trash-2" aria-hidden="true"></i></button><?php endif; ?></td>
						</tr>
					<?php endforeach; ?>
