...
if (!function_exists('streamcreed_t')) {
	function streamcreed_t(string $key, string $fallback): string {
		global $language;
...
$streamcreedPrimaryNav = [
	[streamcreed_t('dashboard', 'Dashboard'), 'dashboard', 'fe-activity', [], ['dashboard', 'index'], null],
	[streamcreed_t('live_connections', 'Live Connections'), 'live_connections', 'fe-wifi', ['live_connections'], ['live_connections'], null],
	[streamcreed_t('lines', 'Streaming Lines'), '#', 'fe-menu', ['users'], ['lines', 'line', 'line_mass'], 'lines'],
	[streamcreed_t('users', 'Users'), '#', 'fe-users', ['mng_regusers', 'mass_edit_reguser', 'mng_packages', 'mng_groups'], ['users', 'user', 'user_mass', 'packages', 'package', 'groups', 'group'], 'users'],
	[(is_string($language ?? null) && method_exists($language, 'current') && $language::current() === 'fr') ? 'Appareils' : streamcreed_t('devices', 'Devices'), '#', 'fe-monitor', ['manage_mag', 'manage_e2', 'add_hmac'], ['mags', 'mag', 'enigmas', 'enigma', 'hmacs', 'hmac'], 'devices'],
	[streamcreed_t('content', 'Content'), '#', 'fe-play', ['streams', 'movies', 'series', 'radio', 'categories', 'bouquets', 'epg'], ['streams', 'stream', 'movies', 'movie', 'series', 'serie', 'series_mass', 'episodes', 'episode', 'episodes_mass', 'radios', 'radio', 'stream_categories', 'stream_category', 'bouquets', 'bouquet', 'bouquet_order', 'bouquet_sort', 'epgs', 'epg'], 'content'],
	[streamcreed_t('servers', 'Servers'), '#', 'fe-server', ['servers', 'add_server', 'edit_server', 'server_order', 'process_monitor'], ['servers', 'server', 'server_view', 'server_install', 'server_order', 'proxies', 'proxy', 'process_monitor'], 'servers'],
	[streamcreed_t('logs', 'Logs'), '#', 'fe-clock', ['panel_logs', 'client_request_log', 'login_logs'], ['panel_logs', 'client_logs', 'login_logs', 'user_logs', 'stream_errors'], 'logs'],
	[streamcreed_t('system', 'System'), '#', 'fe-settings', ['settings'], ['settings', 'modules', 'backups', 'cache', 'rtmp_monitor', 'stream_rank', 'theft_detection'], 'system'],
	[streamcreed_t('tickets', 'Tickets Support'), 'tickets', 'fe-help-circle', ['tickets'], ['tickets', 'ticket', 'ticket_view'], null],
];
$streamcreedDrillNav = [
	'lines' => [streamcreed_t('lines', 'Streaming Lines'), [
		['', [[streamcreed_t('add_line', 'Create New Line'), 'line', ['add_user']], [streamcreed_t('manage_lines', 'Manage Lines'), 'lines', ['users']], [streamcreed_t('mass_edit_users', 'Mass Edit Lines'), 'line_mass', ['mass_edit_users']]]],
	]],
	'users' => [streamcreed_t('users', 'Users'), [
		['', [[streamcreed_t('users', 'Registered Users'), 'users', ['mng_regusers']], [streamcreed_t('mass_edit_users', 'Mass Edit Users'), 'user_mass', ['mass_edit_reguser']], [streamcreed_t('packages', 'Packages'), 'packages', ['mng_packages']], [streamcreed_t('groups', 'Groups'), 'groups', ['mng_groups']]]],
	]],
	'devices' => [(is_string($language ?? null) && method_exists($language, 'current') && $language::current() === 'fr') ? 'Appareils' : streamcreed_t('devices', 'Devices'), [
	...
	'content' => [streamcreed_t('content', 'Content'), [
		['', [[streamcreed_t('streams', 'Live Streams'), 'streams', ['streams']], [streamcreed_t('movies', 'VOD Movies'), 'movies', ['movies']], [streamcreed_t('series', 'TV Series'), 'series', ['series']], [streamcreed_t('radio', 'Radio'), 'radios', ['radio']], [streamcreed_t('epg', 'EPG files'), 'epgs', ['epg']], [streamcreed_t('categories', 'Streaming Categories'), 'stream_categories', ['categories']], [streamcreed_t('bouquets', 'Bouquets'), 'bouquets', ['bouquets']]]],
	]],
	'servers' => [streamcreed_t('servers', 'Servers'), [
		['', [[(is_string($language ?? null) && method_exists($language, 'current') && $language::current() === 'fr') ? 'Installer le Loadbalancer' : streamcreed_t('install_load_balancer', 'Install Loadbalancer'), 'server_install', ['add_server']], [streamcreed_t('manage_servers', 'Manage Servers'), 'servers', ['servers']], [streamcreed_t('manage_proxies', 'Manage Proxies'), 'proxies', ['servers']], [streamcreed_t('server_order', 'Server Order'), 'server_order', ['server_order']], [streamcreed_t('process_monitor', 'Process Monitor'), 'process_monitor', ['process_monitor']]]],
	]],
	'logs' => [streamcreed_t('logs', 'Logs'), [['', [[streamcreed_t('panel_logs', 'Panel Logs'), 'panel_logs', ['panel_logs']], [streamcreed_t('client_logs', 'Client Logs'), 'client_logs', ['client_request_log']], [streamcreed_t('login_logs', 'Login Logs'), 'login_logs', ['login_logs']], [streamcreed_t('user_logs', 'User Logs'), 'user_logs', ['reg_userlog']]]]]],
	'system' => [streamcreed_t('system', 'System'), [
...
$streamcreedTranslationMap = [];
$streamcreedLanguageCode = is_string($language ?? null) && method_exists($language, 'current') ? $language::current() : 'en';
if ($streamcreedLanguageCode !== 'en') {
	$streamcreedEnglish = parse_ini_file(MAIN_HOME . 'resources/langs/en.ini', false, INI_SCANNER_RAW) ?: [];
	$streamcreedTranslated = parse_ini_file(MAIN_HOME . 'resources/langs/' . $streamcreedLanguageCode . '.ini', false, INI_SCANNER_RAW) ?: [];
	foreach ($streamcreedEnglish as $streamcreedKey => $streamcreedEnglishText) {
		$streamcreedTranslatedText = $streamcreedTranslated[$streamcreedKey] ?? $streamcreedEnglishText;
		if ($streamcreedTranslatedText !== $streamcreedEnglishText && strip_tags($streamcreedEnglishText) === $streamcreedEnglishText && strip_tags($streamcreedTranslatedText) === $streamcreedTranslatedText) {
			$streamcreedTranslationMap[$streamcreedEnglishText] = $streamcreedTranslatedText;
		}
	}
}
$streamcreedCustomTranslations = [
	'fr' => [
		'Account' => 'Compte', 'Profile details' => 'Détails du profil', 'Leave blank to keep your current password' => 'Laissez vide pour conserver votre mot de passe actuel', 'Server default' => 'Valeur par défaut du serveur',
...
foreach ($streamcreedCustomTranslations[$streamcreedLanguageCode] ?? [] as $streamcreedEnglishText => $streamcreedTranslatedText) {
	$streamcreedTranslationMap[$streamcreedEnglishText] = $streamcreedTranslatedText;
}
?>
<!doctype html>
			...
			<header class="sc-topbar">
				...
				<details class="sc-account">
					...
					<div class="sc-account-menu">
						<a href="edit_profile"><i class="fe-user" aria-hidden="true"></i> <?php echo htmlspecialchars(streamcreed_t('edit_profile', 'Profile'), ENT_QUOTES, 'UTF-8'); ?></a>
						<a href="dashboard?admin_ui=legacy"><i class="fe-corner-up-left" aria-hidden="true"></i> <?php echo htmlspecialchars(streamcreed_t('legacy_ui', 'Use legacy UI'), ENT_QUOTES, 'UTF-8'); ?></a>
						<a href="logout"><i class="fe-log-out" aria-hidden="true"></i> <?php echo htmlspecialchars(streamcreed_t('logout', 'Log out'), ENT_QUOTES, 'UTF-8'); ?></a>
					</div>
				</details>
			</header>
