...
if (Authorization::check('adv', 'index')): ?>
			<div class="row">
				<div class="col-12">
					<div class="page-title-box">
						...
						<?php if (!$rMobile): ?>
							<h4 class="page-title">
								<?php if (isset(RequestManager::getAll()['server_id'])): ?>
									<?php echo ServerRepository::getAll()[intval(RequestManager::getAll()['server_id'])]['server_name']; ?>&nbsp;
									<a href='server_view?id=<?php echo intval(RequestManager::getAll()['server_id']); ?>' class='btn btn-light waves-effect waves-light btn-xs tooltip-right' title='View Server'><i class='mdi mdi-chart-line'></i></a>
									<a href='process_monitor?server=<?php echo intval(RequestManager::getAll()['server_id']); ?>' class='btn btn-light waves-effect waves-light btn-xs tooltip-right' title='Process Monitor'><i class='mdi mdi-chart-bar'></i></a>
								<?php else: ?>
									Dashboard
								<?php endif; ?>
							</h4>
			...
			<div class="row mb-4">
				<div class="col-md-4">
					<?php if (Authorization::check('adv', 'live_connections')): ?><a href="./live_connections"><?php endif; ?>
	...
	if (!$rMobile) {
		if ($rSettings['dashboard_stats']) {
			...
			if (isset(RequestManager::getAll()['server_id'])) {
				echo '&server_id=' . intval(RequestManager::getAll()['server_id']);
			}
