(function () {
	...
	if (passwordInput) {
		...
		passwordInput.addEventListener('input', function () { var value = passwordInput.value, score = (value.length >= 10 ? 1 : 0) + (/[A-Z]/.test(value) ? 1 : 0) + (/[0-9]/.test(value) ? 1 : 0) + (/[^A-Za-z0-9]/.test(value) ? 1 : 0), labels = ['Too weak', 'Weak', 'Medium', 'Strong', 'Very strong']; meterBar.style.width = Math.max(8, score * 25) + '%'; meterLabel.textContent = value ? labels[score] : ''; meterBar.dataset.score = score; }); passwordInput.dispatchEvent(new Event('input'));
	}
