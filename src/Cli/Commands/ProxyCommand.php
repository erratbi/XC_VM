...
class ProxyCommand implements CommandInterface {
	...
	public function getDescription(): string {
		return 'Proxy — MPEG-TS stream proxying via sockets';
	}
	...
	private function startProxy($rStreamID, $rStreamInfo, $rStreamArguments, &$rFP): void {
		global $db;
		...
		$rFP = $this->getActiveStream($rURLs, $rContext);
		if (!is_resource($rFP)) {
	...
	private function getSockets($rStreamID): array {
		$rSockets = array();
	...
	private function getActiveStream($rURLs, $rContext) {
